...
class lloyd_gunet(torch.nn.Module):
    def __init__(self, lvl, num_layers, dim_embed, K = 2, ratio = 0.5, lr =1e-3):
        ...
        self.convs_up = torch.nn.ModuleDict()
        self.f2c = torch.nn.ModuleDict()
        self.c2f = torch.nn.ModuleDict()
        
        for layer in range(num_layers):
            self.c2f[str(layer)] = torch.nn.ModuleList()
            self.f2c[str(layer)] = torch.nn.ModuleList()
            self.convs_down[str(layer)] = torch.nn.ModuleList()
            self.convs_up[str(layer)] = torch.nn.ModuleList()
        ...
        for layer in range(num_layers):
            for i in range(lvl):
                ...
                if i != lvl-1:
                    ...
                    self.f2c[str(layer)].append(conv_fc)            
                ...
                if i != 0:
                    ...
                    self.c2f[str(layer)].append(conv_cf)
                ...
                if i!=lvl-1:
                    ...
                    self.convs_down[str(layer)].append(conv_ff_down)
                ...
                if i != 0:
                    ...
                    self.convs_up[str(layer)].append(conv_ff_up)
    ...
    def forward(self, grid, train):
        ...
        for layer in range(self.num_layers):
            ...
            for i in list_level:
                ...
                out = self.convs_down[str(layer)][i](x_ff, edge_index_dict, edge_attr_dict)
                # out_ = self.down_conv_features[layer*(self.lvl-1) + i](out['L'+str(i)])

                x_ff['L'+str(i)] = out['L'+str(i)]
 
                out = self.f2c[str(layer)][i](x_ff, edge_index_dict)#, edge_attr_dict)
            ...
            for j, i in enumerate(list_level):
                ...
                out = self.c2f[str(layer)][j](x_ff, edge_index_dict)#, edge_attr_dict)
                x_ff['L'+str(i-1)] = out['L'+str(i-1)]
                out = self.convs_up[str(layer)][j](x_ff, edge_index_dict, edge_attr_dict)
